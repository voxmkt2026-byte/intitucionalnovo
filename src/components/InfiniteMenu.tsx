...
function makeVertexArray(gl: WebGL2RenderingContext, bufLocNumElmPairs: [WebGLBuffer, number, number][], indices?: number[] | Uint16Array) {
...
class InfiniteGridMenu {
  ...
  constructor(canvas: HTMLCanvasElement, items: MenuItem[], onActiveItemChange: (i: number) => void, onMovementChange: (m: boolean) => void, onInit: ((sk: InfiniteGridMenu) => void) | null, scale = 1.0) {
    this.canvas = canvas;
    this.items = items || [];
  ...
  init(onInit: ((sk: InfiniteGridMenu) => void) | null) {
    ...
    this.instancePositions = this.icoGeo.vertices.map((v: Vertex) => v.position);
  ...
  animate(deltaTime: number) {
    ...
    const positions = this.instancePositions.map((p: vec3) => vec3.transformQuat(vec3.create(), p, this.control.orientation));
    const scale = 0.25;
...
export default function InfiniteMenu({ items, scale = 1.0, onActiveIndexChange }: InfiniteMenuProps) {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      ...
      if (onActiveIndexChange) onActiveIndexChange(itemIndex);
    };
