...
async function resolveFont(font: string, fontUrl?: string) {
  ...
  if (!effectiveUrl) {
    if (document.fonts && (document as any).fonts.load) {
      try {
  ...
  try {
    ...
    if (document.fonts && (document as any).fonts.load) {
      try { await (document as any).fonts.load(resolved); } catch { /* ignore */ }
    }
    return resolved;
  } catch (error) {
...
class Title {
  ...
  createMesh() {
    ...
    const textHeightVal = this.plane.scale.y * 0.15;
    const textWidth = textHeightVal * aspect;
    this.mesh.scale.set(textWidth, textHeightVal, 1);
    this.mesh.position.y = -this.plane.scale.y * 0.5 - textHeightVal * 0.5 - 0.05;
...
class App {
  ...
  constructor(container: HTMLElement, { items, bend, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05 }: {
    items?: any[];
    bend?: number;
    textColor?: string;
    borderRadius?: number;
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    resolveFont(font, fontUrl).then(resolvedFont => {
      ...
      // Pause WebGL rAF when off-screen (massive perf win on mobile)
      observer = new IntersectionObserver(
        ([entry]) => {
          ...
          if (entry.isIntersecting) {
            // Resume
            if (!app.raf) {
              app.raf = window.requestAnimationFrame(app.update.bind(app));
            }
          } else {
            // Pause
            if (app.raf) {
